...
def convert_aud_to_usd(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Build a date-string → rate map with forward fill then back fill so
    # weekends/holidays inherit the previous trading day's rate.
    ...
    # Verify every AUD transaction date has a rate before applying. A 1.0
    # fallback would silently leave AUD values, which is worse than failing.
    missing = [d for d in all_dates if d not in rate_map]
    if missing:
        raise RuntimeError(
            f"FX rate missing for {len(missing)} date(s) (e.g. {missing[0]}); "
            f"AUDUSD=X coverage incomplete"
        )
    ...
    for col in ["price", "fees", "net_amount"]:
        if col in df.columns:
            df.loc[mask, col] = df.loc[mask].apply(
                lambda r, c=col: r[c] * rate_map[r["date"].strftime("%Y-%m-%d")],
                axis=1,
            )
